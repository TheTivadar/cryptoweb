...
const AiBotTypeCard = ({ title, description, color, percentage }: { title: string, description: string, color: number, percentage:number }) => {
    return (
        <div className='rounded-[20px] w-full h-24 border border-purple/40 bg-stone-950 relative overflow-hidden '>
            {
                color === 1 ?
                    <div className={`absolute inset-0 w-full h-full bg-gradient-to-b from-stone-950 via-stone-950 from-50% to-yellow-800/50`} />
                    : color === 2 ?
                        <div className={`absolute inset-0 w-full h-full bg-gradient-to-b from-stone-950 via-stone-950 from-50% to-blue-800/50`} />
                        :
                        <div className={`absolute inset-0 w-full h-full bg-gradient-to-b from-stone-950 via-stone-950 from-50% to-violet-800/50`} />
            }

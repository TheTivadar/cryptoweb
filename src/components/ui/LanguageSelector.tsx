...
export function LanguageSelectorNavMobile() {
    const locale = useLocale();
    const router = useRouter();
    const pathname = usePathname();
    ...
    return (
        <DropdownMenu  >
            <DropdownMenuTrigger asChild>
                <button className="h-full text-base flex py-[7px] rounded-full group  text-white items-center gap-2 px-3  bg-black border-2 border-white/[0.05] backdrop-blur-sm focus:transparent">
                    <ReactCountryFlag
                        countryCode={countryCodes[locale as keyof typeof countryCodes]}
                        svg
                        style={{
            ...
            <DropdownMenuContent className="w-14 bg-black rounded-[10px] backdrop-blur-sm shadow-lg flex flex-col items-center justify-center gap-1">
                {locales.map((locale) => (
                    <DropdownMenuItem
                        key={locale.id}
                        onClick={() => handleLanguageChange(locale.language)}
                        className="flex gap-2 items-center w-full justify-center text-lg"
                    >
                        <ReactCountryFlag 
                            countryCode={countryCodes[locale.language as keyof typeof countryCodes]}
                            svg

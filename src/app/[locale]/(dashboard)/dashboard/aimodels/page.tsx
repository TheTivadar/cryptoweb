    ...
    return (
        <div className='mx-[1vw] pt-[2vh]'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:flex  lg:flex-row items-center gap-6'>
                <ModelCards userBalances={user.userBalances} userId={user.id} title={"Biztonságos"} currentBalance={(user.userBalances?.find(b => b.type === 'SAFE')?.amount || 0)} percentage={Math.round(((user.userBalances?.find(b => b.type === 'SAFE')?.amount || 0) / (user.balance || 1)) * 100)} description2={"Alacsony kockázatú, stabil hozamot biztosító kereskedéseket hajt végre, és szigorú szabályokat használ a negatív kereskedés elkerülése érdekében."} description={"Kapcsold hozzá Phantom vagy Metamask fiókodat az oldalhoz, hogy amikor forgalomba kerül a saját tokenünk ingyenes részesülj belőle."} color={1} className={"bg-yellow-300/80"} />
                <ModelCards userBalances={user.userBalances} userId={user.id} title={"Normál"} currentBalance={(user.userBalances?.find(b => b.type === 'NORMAL')?.amount || 0)} percentage={Math.round(((user.userBalances?.find(b => b.type === 'NORMAL')?.amount || 0) / (user.balance || 1)) * 100)} description2={"Kiegyensúlyozott, közepes kockázatú stratégiát követ. Kisebb kockázatokat is megenged, hogyha ezáltal később profitot tud termelni."} description={"Kapcsold hozzá Phantom vagy Metamask fiókodat az oldalhoz, hogy amikor forgalomba kerül a saját tokenünk ingyenes részesülj belőle."} color={2} className={"bg-blue-300"} />
                <ModelCards userBalances={user.userBalances} userId={user.id} title={"Agresszív"} currentBalance={(user.userBalances?.find(b => b.type === 'RISKY')?.amount || 0)} percentage={Math.round(((user.userBalances?.find(b => b.type === 'RISKY')?.amount || 0) / (user.balance || 1)) * 100)} description2={"Kifejezetten a gyors és nagy hozam elérésére lett kialakítva. Magasabb kockázattú coinokkal dolgozik a nagyobb volumen érdekébben, és intenzív napi kereskedéseket végez."} description={"Kapcsold hozzá Phantom vagy Metamask fiókodat az oldalhoz, hogy amikor forgalomba kerül a saját tokenünk ingyenes részesülj belőle."} color={3} className={"bg-purple"} />
            </div>
        </div>
    )
}
...
const ModelCards = ({ title, currentBalance, percentage, description, description2, color, userId, className, userBalances }: { userBalances: UserBalances[], title: string, currentBalance: number, percentage: number, description: string, description2: string, color: number, userId: string, className: string }) => {
    return (
        <Card className=' w-full rounded-[30px]  border-purple/50 relative overflow-hidden h-auto '>
            ...
            <div className='z-20 relative flex flex-col p-4'>
                ...
                <Separator className='h-[2px] mt-6 bg-white/30' />
                <p className='text-white/80 pt-4'> {description2} </p>
                <Separator className='h-[2px] mt-6 bg-white/30' />
